"""Create the manuscript's level-wise and factor-wise forest plots.

Inputs are the selected-model tables written by
``04_final_logistic_regression.R``. The significance filters, ordering,
reference labels, display labels, and figure dimensions preserve the original
plotting workflow; only paths and runtime setup have been made portable.
"""
...
def _available_font() -> str:
    """Prefer Arial while avoiding noisy warnings on systems without it."""
...
def forest_plot(data: pd.DataFrame,
    ...
    if category_map is None:
        ...
        ax.set_yticklabels(order,fontdict={"family": PLOT_FONT}, fontsize=17)
        ax.invert_yaxis()
        ax.set_xlabel("Odds Ratio",fontdict={"family": PLOT_FONT})
        ax.set_title(title,fontdict={"family": PLOT_FONT})
        if logx:
            ax.set_xscale("log")
        if len(models) > 1:
            ...
            fp_label = fm.FontProperties(family=PLOT_FONT, size=6)
            fp_title = fm.FontProperties(family=PLOT_FONT, weight='bold', size=7)
    ...
    ax.axvline(1.0, linestyle="--", linewidth=1, alpha=0.6)
    ax.set_yticks(range(n))
    ax.set_yticklabels(y_order,fontdict={"family": PLOT_FONT}, fontsize=17)
    ax.invert_yaxis()
    ax.set_xlabel("Odds Ratio",fontdict={"family": PLOT_FONT}, fontsize=20)
    ax.set_title(title)
    #print(y_order)
...
Level_Labels = {
    ...
    'MAIN_METHOD8': 'Other Chemical Drugs',
...
def _selected_model_label() -> str:
    """Build a model label from the final-model metadata when available."""
    metadata_path = TABLE_DIR / "selected_model_metadata.csv"
    if metadata_path.exists():
        metadata = pd.read_csv(metadata_path)
        if not metadata.empty and "analysis_n" in metadata:
            return f"Subset 3 (Cut_2; N={int(metadata.loc[0, 'analysis_n']):,})"
    return "Subset 3 (Cut_2)"


def main() -> None:
    or_path = TABLE_DIR / "or_table_Cut_2_DF_50Columns.xlsx"
    var_path = TABLE_DIR / "var_tests_Cut_2_DF_50Columns.xlsx"
    missing = [path for path in (or_path, var_path) if not path.exists()]
    if missing:
        raise FileNotFoundError(
            "Final-model table(s) not found. Run 04_final_logistic_regression.R "
            f"first: {', '.join(str(path) for path in missing)}"
        )

    FIGURE_DIR.mkdir(parents=True, exist_ok=True)
    model_label = _selected_model_label()
    level_or_files = {model_label: str(or_path)}
    factor_var_files = {model_label: str(var_path)}
    factor_or_files = {model_label: str(or_path)}
    category_order_level = ["Demographics", "Context", "Warning Signs", "MetaInfo"]

    make_levelwise_forest(
        or_files=level_or_files,
        common_only=False,
        top_k=800,
        out_png=str(FIGURE_DIR / "demographic_forest_table.png"),
        category_map=category_map_level,
        category_order=category_order_level,
        label_map=Level_Labels,
        within_order="none",
        label_order=my_label_order,
    ...
    print(f"Outputs saved in: {FIGURE_DIR}")
